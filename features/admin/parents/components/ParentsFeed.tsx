...
import DetailParentModal from "./DetailParentModal";
import { usePagination } from "@/hooks/usePagination";
import { getAllParents } from "@/services/parent.service";

export default function ParentsFeed() {
  ...
  const fetchParentsData = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await getAllParents(page, limit, searchQuery);
      ...
      <div className="bg-white rounded-[16px] border border-border-input/40 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-border-input/30 flex justify-between items-center bg-white gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Cari nama, email, atau no. telepon orang tua..."
        ...
        <div className="relative min-h-[300px]">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {!isLoading && parentsData.length === 0 ? (
                <tr>
                  <td
                    colSpan={5}
                  ...
                  >
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-4">
                        <div className="w-9 h-9 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 text-[13px] font-bold shrink-0">
                          {row.name
                    ...
                    <td className="px-6 py-5">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[13px] text-icon-muted">
                          {row.phone_number || "-"}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-[15px] text-icon-muted">
                      {row.clinic?.name || "Belum Diatur"}
                    </td>
                    <td className="px-6 py-5">
                      <div className="inline-flex justify-center min-w-[100px] py-1.5 rounded-full text-[13px] font-semibold bg-success-bg/50 text-success-text">
                        Aktif
                      </div>
                    </td>
                    <td className="px-6 py-5">
        ...
        <div className="px-6 py-4 border-t border-border-input/30 flex items-center justify-between bg-white">
          <span className="text-sm text-icon-muted">
            ...
            {Math.min(page * limit, totalItems)} dari {totalItems} pengguna
          </span>
