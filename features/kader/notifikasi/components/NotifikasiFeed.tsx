...
import { ArrowLeft, CircleAlert, Calendar, FileText, Settings, CheckCircle2 } from "lucide-react";
import { useRouter } from "next/navigation";
import { NotifikasiItem } from "../types";

export default function NotifikasiFeed({ initialData }: { initialData: NotifikasiItem[] }) {
    ...
    switch (item.tipe) {
      case "peringatan":
        return {
          ...
          icon: <CircleAlert size={26} className="text-white fill-red-700" strokeWidth={1.5} />,
        };
      case "jadwal":
        return {
      ...
      case "sistem":
      default:
        const iconNode = item.ikonSistem === "check" 
          ? <CheckCircle2 size={26} className="text-white fill-gray-500" strokeWidth={1.5} />
          : <Settings size={22} className="text-white fill-gray-600" strokeWidth={1.5} />;
        return {
          ...
          return (
            ...
            >
              <div className="p-4 flex items-start gap-4 w-full">
                {/* Ikon */}
                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${style.iconWrapper}`}>
                  {style.icon}
                </div>
