...
export function transformApiToMetrics(
  childInfo: any,
  apiGraphData: any,
  rawMeasurementsArray: any[] = [],
) {
  const rawMeasurements = Array.isArray(rawMeasurementsArray)
    ? rawMeasurementsArray
    : [];

  const riwayat = rawMeasurements.map((m: any) => {
