...
import { SOLUTIONS_DATA } from "../constants/landing";
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-sm font-bold text-btn-primary tracking-widest uppercase mb-3">
            Solusi Terintegrasi di Ekosistem Kesehatan
          </h2>
          <h3 className="text-3xl md:text-4xl font-bold text-text-main max-w-3xl mx-auto leading-tight">
            Menghubungkan orang tua, posyandu, dan tenaga kesehatan dalam satu
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {SOLUTIONS_DATA.map((solution, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-white rounded-[24px] p-8 border border-border-input/40 shadow-[0_8px_30px_rgba(0,0,0,0.03)] flex flex-col h-full hover:-translate-y-2 transition-transform duration-300"
              >
                <div className="w-14 h-14 bg-primary-light rounded-2xl flex items-center justify-center mb-6">
                  <Icon
                    className="text-btn-primary w-7 h-7"
                    strokeWidth={2.5}
                  />
                </div>
                ...
                <Link
                  href={solution.href}
                  className="mt-auto text-sm font-bold text-btn-primary flex items-center gap-1 hover:gap-2 transition-all"
                >
                  {solution.actionText}
                  <svg
                    width="16"
